...
class CreateServerForm extends StatefulWidget {
  const CreateServerForm({super.key});
...
class _CreateServerFormState extends State<CreateServerForm> {
  ...
  Future<void> _createServer() async {
    ...
    final doc = FirebaseFirestore.instance.collection('config').doc('server');
    ...
    // Generate new key like server_3
    final serverKeys =
        currentData.keys
            .where((key) => key.startsWith('server_'))
            .map((key) => int.tryParse(key.split('_').last) ?? 0)
            .toList();

    int nextIndex =
        serverKeys.isNotEmpty
            ? (serverKeys.reduce((a, b) => a > b ? a : b) + 1)
            : 1;
    final newKey = 'server_$nextIndex';
    ...
    // Add to Firestore
    await doc.set({newKey: newServerData}, SetOptions(merge: true));

    setState(() => _isLoading = false);

    Get.back(); // Go back to servers list
    Get.snackbar('Success', 'Server created successfully');
  }
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            // Active checkbox
            Text('Make your Server Active or Inactive'),
            CheckboxListTile(
